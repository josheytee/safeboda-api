...
class PromoCodeRetrievalTest extends TestCase
{
    ...
    public function testReturnAllPromoCodes()
    {
        ...
        $this->call("GET", route('promo-code-deactivate', ['code' => $promoCodes[rand(0, 9)]->code]));
        $this->call("GET", route('promo-code-deactivate', ['code' => $promoCodes[rand(0, 9)]->code]));
        $response = $this->get(route('promo-codes', ['type' => 'all']));
        $response->seeJsonStructure([
            'promoCodes' => ['*' => [
                'id', 'code', 'radius', 'amount', 'created_at', 'expires_at', 'deleted_at'
            ]]
        ]);
    ...
    public function testReturnActivePromoCodes()
    {
        ...
        $response->seeJsonStructure([
            'promoCodes' => ['*' => [
                'id', 'code', 'radius', 'amount', 'created_at', 'expires_at'
            ]]
        ]);
        $response->assertResponseStatus(200);
    }
